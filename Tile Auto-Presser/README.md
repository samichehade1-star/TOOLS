...
Digit templates (1-4) and controller-icon templates (A/B/X/Y) are already
bundled in, pre-built from real screenshots of the game — **no calibration
...
2. Pick your **Role** (Michael or Civilian). This bounds how many tiles a
   real combo can have for that role (Michael: 4-5, civilian: 3-4) so a
...
3. Click **Use Whole Screen** (one-time).
4. Click **Start Watching**. That's it — go play. Press **F8** anywhere
...
your region + digit/icon templates.
...
3. **Run** (hotkey **F6**) — locks onto whichever known icon it currently
   sees and mashes it continuously at full speed, switching automatically
   only when it confidently detects a genuinely *different* known icon.

   **⚠ It does NOT stop on its own anymore.** Earlier versions stopped as
   soon as the icon looked "gone," but real prompts flicker in and out of
   detection for 100-200ms+ at a stretch (animation, a busy scan region,
   etc.), which kept cutting mashing short mid-puzzle. The fix trades that
   for a rule with a real downside: **toggle F6 ON right before the puzzle
   and OFF right as soon as it's done, every time.** Leaving it running
   between puzzles risks it locking onto a visually similar icon reused by
   an unrelated puzzle (seen live: the same mouse-click icon, but wrapped in
   a timed circular skill-check ring elsewhere in the game) and mashing
   through something that needed one precisely-timed input instead.
...
- `mash_press_hold_ms` / `mash_press_gap_ms` control the spam rate (default
  4ms hold / 2ms gap, ~100-120 presses/sec after Windows' timer-resolution
  floor is accounted for). Push lower if the game keeps up; back off if
...
- `mash_poll_interval_ms` (default 20) is how often the *detector* scans for
...
- `mash_miss_tolerance` (default 15 polls) is how many consecutive
  ...
  considered not-yet-found. Since Run no longer auto-stops on a miss at
  all, this mostly only matters while waiting for the first icon to show up.
...
- Make sure **Role** (Michael/Civilian) matches whoever you're actually
  playing. A mid-animation frame that only shows part of the real combo (or,
  rarely, picks up a stray extra box) gets rejected outright if its tile
  count falls outside that role's real range (Michael: 4-5, civilian: 3-4)
  instead of getting pressed as a bogus short/long combo.
- If it's picking the wrong combo line or ever misfires, or valid tiles are
  ...
  - `confirm_count`: how many consecutive identical readings are required
    before pressing (default 2). Raises this if a combo still gets pressed
...
- If the game's tile/icon styling ever changes and stops matching, delete the
  relevant file in `templates/keyboard/` or `templates/controller/` and use
...
- The F8 hotkey is a low-level keyboard hook — if it doesn't respond while
  the game has focus, try running `TileAutoPresser.exe` as administrator,
  which is usually only needed if the game itself runs elevated.
