...
const translations = {
    english: {
        ...
        "settings-section-trainer-spoofer": "tools",
        "settings-section-trainer-spoofer-subtitle": "folder paths — the exe inside each folder is launched directly",
        "trainer-folder-path-label": "trainer folder (Visenya X):",
        "trainer-folder-hint": "folder containing the trainer's .exe. Only Visenya has a trainer — leave this empty and the trainer button won't show up.",
        "spoofer-folder-path-label": "spoofer folder:",
        "spoofer-folder-hint": "folder containing the spoofer's .exe — point this at either your Visenya S folder or your Nyxia spoofer folder, whichever you use.",
        "pakbypass-folder-path-label": "pak bypass folder (Nyxia):",
        "pakbypass-folder-hint": "folder containing Nyxia's pak bypass .exe.",
        "unlocker-folder-path-label": "unlocker folder (Nyxia):",
        "unlocker-folder-hint": "folder containing Nyxia's unlocker .exe.",
        "pakbypass-button": "pak bypass",
        "unlocker-button": "unlocker",
    ...
    russian: {
        ...
        "settings-section-trainer-spoofer": "инструменты",
        "settings-section-trainer-spoofer-subtitle": "пути к папкам — exe внутри каждой папки запускается напрямую",
        "trainer-folder-path-label": "папка трейнера (Visenya X):",
        "trainer-folder-hint": "папка с exe-файлом трейнера. Трейнер есть только у Visenya — оставьте пустым, и кнопка трейнера не появится.",
        "spoofer-folder-path-label": "папка спуфера:",
        "spoofer-folder-hint": "папка с exe-файлом спуфера — укажите папку Visenya S или папку спуфера Nyxia, в зависимости от того, что вы используете.",
        "pakbypass-folder-path-label": "папка pak bypass (Nyxia):",
        "pakbypass-folder-hint": "папка с exe-файлом pak bypass от Nyxia.",
        "unlocker-folder-path-label": "папка unlocker (Nyxia):",
        "unlocker-folder-hint": "папка с exe-файлом unlocker от Nyxia.",
        "pakbypass-button": "pak bypass",
        "unlocker-button": "unlocker",
    ...
    german: {
        ...
        "settings-section-trainer-spoofer": "werkzeuge",
        "settings-section-trainer-spoofer-subtitle": "ordnerpfade — die exe im jeweiligen ordner wird direkt gestartet",
        "trainer-folder-path-label": "trainer-ordner (Visenya X):",
        "trainer-folder-hint": "ordner mit der trainer-exe. Nur Visenya hat einen trainer — leer lassen, dann erscheint der trainer-button nicht.",
        "spoofer-folder-path-label": "spoofer-ordner:",
        "spoofer-folder-hint": "ordner mit der spoofer-exe — zeige hier entweder auf deinen Visenya S-ordner oder deinen Nyxia-spoofer-ordner, je nachdem, was du nutzt.",
        "pakbypass-folder-path-label": "pak-bypass-ordner (Nyxia):",
        "pakbypass-folder-hint": "ordner mit der pak-bypass-exe von Nyxia.",
        "unlocker-folder-path-label": "unlocker-ordner (Nyxia):",
        "unlocker-folder-hint": "ordner mit der unlocker-exe von Nyxia.",
        "pakbypass-button": "pak bypass",
        "unlocker-button": "unlocker",
    ...
    spanish: {
        ...
        "settings-section-trainer-spoofer": "herramientas",
        "settings-section-trainer-spoofer-subtitle": "rutas de carpetas — el exe dentro de cada carpeta se ejecuta directamente",
        "trainer-folder-path-label": "carpeta del trainer (Visenya X):",
        "trainer-folder-hint": "carpeta que contiene el .exe del trainer. Solo Visenya tiene trainer — déjalo vacío y el botón de trainer no aparecerá.",
        "spoofer-folder-path-label": "carpeta del spoofer:",
        "spoofer-folder-hint": "carpeta que contiene el .exe del spoofer — apunta a tu carpeta de Visenya S o a tu carpeta de spoofer de Nyxia, la que uses.",
        "pakbypass-folder-path-label": "carpeta de pak bypass (Nyxia):",
        "pakbypass-folder-hint": "carpeta que contiene el .exe de pak bypass de Nyxia.",
        "unlocker-folder-path-label": "carpeta del unlocker (Nyxia):",
        "unlocker-folder-hint": "carpeta que contiene el .exe del unlocker de Nyxia.",
        "pakbypass-button": "pak bypass",
        "unlocker-button": "unlocker",
    ...
    chinese: {
        ...
        "settings-section-trainer-spoofer": "工具",
        "settings-section-trainer-spoofer-subtitle": "文件夹路径 — 直接启动文件夹内的exe",
        "trainer-folder-path-label": "修改器文件夹 (Visenya X)：",
        "trainer-folder-hint": "包含修改器.exe的文件夹。只有Visenya有修改器 — 留空则不会显示修改器按钮。",
        "spoofer-folder-path-label": "欺骗器文件夹：",
        "spoofer-folder-hint": "包含欺骗器.exe的文件夹 — 指向你的Visenya S文件夹或Nyxia欺骗器文件夹，取决于你使用哪个。",
        "pakbypass-folder-path-label": "pak bypass 文件夹 (Nyxia)：",
        "pakbypass-folder-hint": "包含Nyxia的pak bypass .exe的文件夹。",
        "unlocker-folder-path-label": "unlocker 文件夹 (Nyxia)：",
        "unlocker-folder-hint": "包含Nyxia的unlocker .exe的文件夹。",
        "pakbypass-button": "pak bypass",
        "unlocker-button": "unlocker",
...
// Delays calling fn until `wait` ms after the last call — used on search
// inputs whose handler re-fetches from the main process (a real disk scan)
// on every keystroke; without this, fast typing fires a full rescan+re-render
// per character instead of once after the user pauses.
...
// Shows/hides each tool's sidebar button based on whether its folder path is
// configured — this is how the sidebar adapts to Visenya vs Nyxia (or any mix
// of the two) without an explicit "which provider" setting: no trainer folder
// set means no trainer button, since Nyxia doesn't have one at all.
function updateToolButtonVisibility() {
    const toggle = (btnId, hasPath) => {
        const btn = document.getElementById(btnId);
        if (btn) btn.style.display = hasPath ? '' : 'none';
    };
    toggle('launch-trainer-btn', !!currentSettings.trainerFolderPath);
    toggle('launch-spoofer-btn', !!currentSettings.spooferFolderPath);
    toggle('launch-pakbypass-btn', !!currentSettings.pakBypassFolderPath);
    toggle('launch-unlocker-btn', !!currentSettings.unlockerFolderPath);
}
...
async function loadSettings() {
    ...
    document.getElementById('trainer-folder-path').value = currentSettings.trainerFolderPath || '';
    document.getElementById('spoofer-folder-path').value = currentSettings.spooferFolderPath || '';
    document.getElementById('pakbypass-folder-path').value = currentSettings.pakBypassFolderPath || '';
    document.getElementById('unlocker-folder-path').value = currentSettings.unlockerFolderPath || '';
...
async function saveSettings() {
    ...
    const settingsToSave = {
        ...
        trainerFolderPath: document.getElementById('trainer-folder-path').value,
        spooferFolderPath: document.getElementById('spoofer-folder-path').value,
        pakBypassFolderPath: document.getElementById('pakbypass-folder-path').value,
        unlockerFolderPath: document.getElementById('unlocker-folder-path').value,
        gameLaunchPaths: {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // tool folder pickers: point each at the folder the tool's exe lives
        // in (Visenya's exe names are fixed, Nyxia's vary, so we just launch
        // whatever single .exe is in the folder rather than require a name)
        const toolFolderBrowseButtons = [
            ['select-trainer-folder', 'trainer-folder-path'],
            ['select-spoofer-folder', 'spoofer-folder-path'],
            ['select-pakbypass-folder', 'pakbypass-folder-path'],
            ['select-unlocker-folder', 'unlocker-folder-path']
        ];
        toolFolderBrowseButtons.forEach(([btnId, inputId]) => {
        ...
        // tool quick-launch buttons: launch whatever .exe is sitting in the
        // configured folder for that tool, once. A click while one is
        // already in flight is ignored instead of queuing a second launch.
        const activeToolLaunches = { trainer: false, spoofer: false, 'pak bypass': false, unlocker: false };

        async function launchTool(button, launchFn, label) {
            if (activeToolLaunches[label]) return;
            ...
            try {
                ...
                if (result.success) {
                    log(`${label} launched.`, 'success');
                } else {
                    log(`failed to launch ${label}: ${result.error}`, 'error');
                }
            } finally {
        ...
        document.getElementById('launch-pakbypass-btn')?.addEventListener('click', (e) => {
            launchTool(e.currentTarget, () => window.electronAPI.launchPakBypass(), 'pak bypass');
        });
        ...
        document.getElementById('launch-game-btn')?.addEventListener('click', async (e) => {
            ...
            showMessageModal(
                ...
                'do not launch the game again — there is a 10 second delay before it starts, to make sure any tools you launched (trainer/spoofer/pak bypass/unlocker) finish injecting first. clicking launch again during the countdown will not speed this up.'
            );
        ...
        document.getElementById('available-mods-search')?.addEventListener('input', debounce(() => renderAvailableModsPicker(), 250));
        ...
        document.getElementById('mod-library-search')?.addEventListener('input', debounce(() => renderInstalledMods(), 250));
