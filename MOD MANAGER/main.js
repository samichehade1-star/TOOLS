...
// Several of the archive libraries above (zip/7z/rar/WASM) each register their
// own process-level exit/error cleanup hooks, which reliably pushes past
// Node's default limit of 10 and spams "MaxListenersExceededWarning" on every
// launch — confirmed one-time at startup, not a growing leak from repeated
// actions, so raising the ceiling is the correct fix rather than a fragile
// workaround.
...
// --- paths ---
const appDataPath = path.join(os.homedir(), 'AppData', 'Local', 'Programs', '~chubmods');
const legacyAppDataPath = path.join(os.homedir(), 'AppData', 'Roaming', 'disobey.top');
const legacyAppDataPath2 = path.join(os.homedir(), 'AppData', 'Local', 'Programs', '~cheesehubmods');
...
let appSettings = {
    ...
    // folders containing each tool's exe — launched directly, no extraction.
    // Visenya's exe names are fixed, Nyxia's vary per install, so whichever
    // single .exe sits in the folder is the one that gets launched. Leaving
    // a path empty hides that tool's button entirely (e.g. Nyxia users have
    // no trainer, so they just never set trainerFolderPath).
    trainerFolderPath: '', // Visenya X
    spooferFolderPath: '', // Visenya S or Nyxia's spoofer — whichever folder is set
    pakBypassFolderPath: '', // Nyxia only
    unlockerFolderPath: '', // Nyxia only
...
function createWindow() {
    ...
    mainWindow = new BrowserWindow({
        ...
        // Electron paints a plain white window the instant it's created,
        // before index.html's CSS has loaded — on a dark UI that shows up as
        // a jarring white flash. backgroundColor covers the gap with the
        // app's own dark background instead of white; show:false + waiting
        // for ready-to-show goes further and skips showing the window at all
        // until the page has actually painted a frame, so there's nothing to
...
app.whenReady().then(async () => {
    ...
    // one-time migration: if this is the first launch under the new data
    // folder and an older one still has data, copy it over so existing
    // users don't lose settings, installed mods, or profiles. Checks the
    // most recent legacy location first (the CheeseHub-era folder), then
    // falls back further to the original disobey.top-era folder.
    try {
        const newHasData = await fs.pathExists(path.join(appDataPath, 'settings.json'));
        if (!newHasData) {
            const cheesehubHasData = await fs.pathExists(path.join(legacyAppDataPath2, 'settings.json'));
            const disobeyHasData = await fs.pathExists(path.join(legacyAppDataPath, 'settings.json'));
            if (cheesehubHasData) {
                await fs.copy(legacyAppDataPath2, appDataPath, { overwrite: false });
                logToConsole(`migrated app data from ${legacyAppDataPath2} to ${appDataPath}.`, 'success');
            } else if (disobeyHasData) {
                await fs.copy(legacyAppDataPath, appDataPath, { overwrite: false });
                logToConsole(`migrated app data from ${legacyAppDataPath} to ${appDataPath}.`, 'success');
            }
        }
    } catch (migrateErr) {
        logToConsole(`app data migration skipped: ${migrateErr.message}`, 'warn');
    }
...
ipcMain.handle('open-discord-invite', async () => {
    const url = 'https://discord.gg/cheesehub';
    logToConsole(`opening discord invite: ${url}`, 'info');
    // shell.openExternal hands this off to the OS's registered handler for
    // discord.gg links — if the Discord app is installed, it typically
    // registers itself for these and this opens directly in-app rather than
    // a browser tab.
 ...
 * the running window). Used to build a full timeline of a tool launch
 * attempt that survives after the fact, regardless of whether anyone
...
// session/license token — two live copies fighting over the same token is a
// very plausible reason a perfectly good launch gets killed a few seconds
// after a second click, since the verification check only confirms
// "something is running in this folder," not "the specific instance THIS
// click started."
const activeToolRuns = new Map(); // folderPath -> runningAs

// Thrown for permanent config problems (folder not set, no exe in it, etc.)
class NonRetryableError extends Error {}

/**
 * Launch whatever single .exe is sitting in a configured tool folder —
 * Visenya's exes have fixed names, Nyxia's vary per install, so rather than
 * hardcode a filename this just launches whatever's in the folder the user
 * pointed us at. Launches once — no automatic retrying on failure.
 */
async function attemptLaunchFromFolder(folderPath, label) {
    await appendDebugLog(`=== ${label} launch started ===`);
    if (!folderPath) {
        await appendDebugLog(`FAIL: no folder path configured`);
        throw new NonRetryableError(`set the ${label} folder path in settings first.`);
    }
    if (!(await fs.pathExists(folderPath))) {
        await appendDebugLog(`FAIL: folder not found at ${folderPath}`);
        throw new NonRetryableError(`${label} folder not found at: ${folderPath}`);
    }

    const trackedRunningAs = activeToolRuns.get(folderPath);
    if (trackedRunningAs) {
        const stillAlive = await findRunningProcessInDir(folderPath, 1);
        if (stillAlive) {
        ...
        activeToolRuns.delete(folderPath); // it actually exited — stale entry, clear it
    }

    const dirEntries = await fs.readdir(folderPath);
    const exeName = dirEntries.find(f => f.toLowerCase().endsWith('.exe'));
    if (!exeName) {
        await appendDebugLog(`FAIL: no .exe found in ${folderPath}`);
        throw new NonRetryableError(`no .exe found inside the ${label} folder: ${folderPath}`);
    }
    const exePath = path.join(folderPath, exeName);
    await appendDebugLog(`launching ${exePath}`);
    logToConsole(`launching ${label} (as administrator): ${exePath}`, 'info');
    await launchElevated(exePath, folderPath, label);
    await appendDebugLog(`launchElevated() returned without error`);

    // No post-launch verification here on purpose — polling for the process
    // to show up produced false-negative errors (tool had actually launched
    // fine, just not detected within the poll window) with nothing wrong to
    // actually report. Still updates activeToolRuns in the background,
    // unawaited, purely so the "already running" duplicate-launch guard
    // above has something to check — this does NOT affect success/failure.
    (async () => {
        for (let i = 0; i < 24; i++) {
            await new Promise(r => setTimeout(r, 1000));
            const name = await findRunningProcessInDir(folderPath, 1);
            if (name) { activeToolRuns.set(folderPath, name); } else { activeToolRuns.delete(folderPath); break; }
        }
    })();

    logToConsole(`${label} launched.`, 'success');
    return { exePath, confirmation: `${label} launched.` };
}
...
/** Scheduled Task names can't contain \/:*?"<>|, so labels get sanitized into one. */
function elevatedTaskName(label) {
    return `ModManagerAutoLaunch_${label.replace(/[^a-zA-Z0-9]+/g, '_')}`;
}

/**
 * The actual script the scheduled task runs, inline as its Argument (rather
 * than the task targeting exePath directly) so the window-activation step
 * below runs AT the task's own elevated integrity level. A plain external
 * "activate that window" call from this (non-elevated) app can't force
 * focus onto a higher-privilege window — Windows blocks that by design —
 * so it has to happen from inside the elevated process itself.
 */
function buildLaunchAndActivateScript(exePath, workingDir) {
    const dirEscaped = workingDir.replace(/'/g, "''");
    return [
        `$p = Start-Process -FilePath ${psQuote(exePath)} -WorkingDirectory ${psQuote(workingDir)} -PassThru`,
...
/**
 * Looks up the powershell Argument string currently registered on the
 * elevated launch task, or null if the task doesn't exist yet.
 */
...
/**
 * Runs a PowerShell script elevated (one real UAC prompt) and waits for it
 * to finish. The script is passed as -EncodedCommand (base64 of UTF-16LE)
 * rather than nested quoting, since it's itself being embedded inside an
 * outer Start-Process -Verb RunAs call. The inner script is expected to
 * `exit 1` on failure so a caught error there actually propagates out here
 * instead of failing silently inside the hidden elevated window.
 */
...
/**
 * Ensures a Scheduled Task exists whose action is powershell.exe running
 * launchArgument (the encoded launch-and-activate script for this exact
 * exePath/workingDir), registered at "highest privileges." Registering a
 * Highest-run-level task requires an elevated caller, so this still needs
 * one real UAC accept — but only the first time for a given tool, or again
 * later if the tracked exe path changes. Once registered, *running* the
 * task via schtasks.exe does NOT prompt again: Windows already has standing
 * consent for it, which is the whole point — the per-launch UAC popup goes
 * away entirely.
 */
async function ensureElevatedTaskRegistered(taskName, launchArgument, label) {
    ...
    if (existing && /powershell(\.exe)?$/i.test(existing.execute) && existing.argument === launchArgument) {
        return; // already registered for this exact exe — no UAC needed
    }
    await appendDebugLog(`${label}: (re)registering elevated launch task "${taskName}"`);
    ...
    await appendDebugLog(`${label}: task "${taskName}" registered`);
}

/**
 * Launch an exe elevated with no per-launch UAC popup, via a Scheduled Task
 * registered at "highest privileges" (see ensureElevatedTaskRegistered).
 * shell.openPath / a plain Start-Process -Verb RunAs would show that popup
 * on every single launch — a lot of trainer/cheat stubs also don't declare
 * an elevation requirement in their own manifest and just silently exit the
 * moment they detect they're not elevated, which looks identical to
 * "nothing happened," so forcing elevation here (rather than trusting the
 * exe to ask for it itself) still matters.
 *
 * The launched tool DOES have a real window (confirmed: title "Launcher",
 * normal on-screen position, not minimized) — it just never gets focus,
 * because Windows blocks a lower-privilege window from stealing foreground
 * away to hand it to a higher-privilege one. It opens directly behind the
 * mod manager and looks invisible unless you Alt-Tab, so the task's own
 * script (buildLaunchAndActivateScript) force-activates the window itself
 * once it's up, from inside the elevated task where that's actually allowed.
 */
async function launchElevated(exePath, workingDir, label) {
    const taskName = elevatedTaskName(label);
    const script = buildLaunchAndActivateScript(exePath, workingDir);
    const encoded = Buffer.from(script, 'utf16le').toString('base64');
    ...
    await new Promise((resolve, reject) => {
        execFile('schtasks.exe', ['/Run', '/TN', taskName], (error, stdout, stderr) => {
            if (error) {
                reject(new Error(`failed to start ${label} via scheduled task: ${(stderr || error.message).trim()}`));
                return;
            }
            resolve();
        });
...
// tool folder-path settings keyed by label — the launch/cancel handlers
// below both go through this, so adding a new tool later is a one-line change
const TOOL_FOLDER_SETTINGS_KEY = {
    'trainer': 'trainerFolderPath',
    'spoofer': 'spooferFolderPath',
    'pak bypass': 'pakBypassFolderPath',
    'unlocker': 'unlockerFolderPath',
};

async function launchConfiguredTool(label) {
    const folderPath = appSettings[TOOL_FOLDER_SETTINGS_KEY[label]];
    try {
        const { confirmation } = await attemptLaunchFromFolder(folderPath, label);
        return { success: true, message: confirmation };
    } catch (error) {
        logToConsole(`error launching ${label}: ${error.message}`, 'error');
        return { success: false, error: error.message };
    }
}

ipcMain.handle('launch-trainer', () => launchConfiguredTool('trainer'));
ipcMain.handle('launch-spoofer', () => launchConfiguredTool('spoofer'));
ipcMain.handle('launch-pakbypass', () => launchConfiguredTool('pak bypass'));
ipcMain.handle('launch-unlocker', () => launchConfiguredTool('unlocker'));
